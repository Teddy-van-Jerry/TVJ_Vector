...
#pragma warning(disable: 26812)// enum type unscoped
	...
	template<typename Elem>
	inline typename const Elem& vector<Elem>::const_iterator::operator*() const
	{
#ifndef NDEBUG
		if (!value)               error_info("Null pointer in operator '*' of const_iterator of tvj::vector.", TVJ_VECTOR_NULLPTR);
		if (value - vec < 0)      error_info("Underflow in operator '*' of const_iterator of tvj::vector.", TVJ_VECTOR_UNDERFLOW);
		if (value - vec >= size_) error_info("Overflow in operator '*' of const_iterator of tvj::vector.", TVJ_VECTOR_OVERFLOW);
#endif
		return *value;
	}
	...
	template<typename Elem>
	inline typename auto vector<Elem>::const_iterator::operator++()
	{
		return ++value;
	}
	...
	template<typename Elem>
	inline typename auto vector<Elem>::const_iterator::operator++(int)
	{
		return value++;
	}
	...
	template<typename Elem>
	inline typename bool vector<Elem>::const_iterator::operator==(const const_iterator& iter) const noexcept
	{
		return this->value == iter.value;
	}
	...
	template<typename Elem>
	inline typename Elem& vector<Elem>::iterator::operator*()
	{
#ifndef NDEBUG
	...
	template<typename Elem>
	inline typename bool vector<Elem>::iterator::operator==(const iterator& iter) const noexcept
	{
